...
## 4. Publish
Create an Epic (or Story) in Jira via Atlassian `createJiraIssue` under the project key in `docs/agents/config.md`, mapping the fields above. Mark it **ready** — set the `ready` triage label, or transition to the `ready_state` status if `config.md` defines one (`getTransitionsForJiraIssue` + `transitionJiraIssue`). Leave it **unassigned** so a human or `fe-flow` can claim it via the ticket protocol.

If the Atlassian MCP is unreachable, follow the fallback in `core-setup`/MCP-SETUP.md (§ "Publish & degraded-mode fallback"): save the full payload to `docs/agents/holding/<date>-pm-to-prd-<KEY>.md` and emit the manual hand-off steps. *Standalone shorthand: write a holding doc and surface the `createJiraIssue` call for the human to run.*

Hand to `pm-to-issues` to break it into vertical slices.
