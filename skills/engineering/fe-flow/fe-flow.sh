#!/usr/bin/env bash
# fe-flow — run one or more ready Jira issues to pre-reviewed PRs, unattended,
...
# skill in your repo (`.claude/skills/fe-flow/fe-flow.sh`) or at the user level
# (`~/.claude/skills/fe-flow/fe-flow.sh`) and the runner is already on disk; run
...
# denied — and the run stalls. fe-flow's very first step reads the Jira issue
...
ship_one() {
  ...
  ( cd "$wt"
    claude -p "Use the fe-flow skill to take Jira issue ${key} to a pre-reviewed PR. Do not merge; stop at the PR for human review." \
      --model "$MODEL" --max-turns "$MAX_TURNS" \
      --permission-mode acceptEdits --allowedTools "$TOOLS" \
      --output-format stream-json --verbose \
      2>&1 | tee "fe-flow-${key}.log"

    # --- cost accounting: pull the final result object from the stream ---
    local res; res="$(grep '"type":"result"' "fe-flow-${key}.log" | tail -1 || true)"
