# Running `fe-flow` headless

`fe-flow` is just the recipe. This is the **runner** — how you execute that recipe with no human in the seat, against **Jira**, while accounting for token cost so efficiency becomes a coaching signal. Start local; the same prompt and skills graduate to CI or cloud later without change.

## The minimal invocation

From inside a target repo (one that has run `core-setup`, with the tracker pointed at Jira in `docs/agents/config.md`):

```bash
claude -p "Use the fe-flow skill to take Jira issue ABC-123 to a pre-reviewed PR. \
...
The canonical runner ships with this skill as [`fe-flow.sh`](fe-flow.sh) — a real, executable file, not a snippet to retype. Each Jira issue runs in its own `git worktree`, so you can ship several at once with no collisions — the Boris Cherny parallel pattern — and each run records exactly what it cost.

**You don't install the runner separately — it ships with the skill.** Wherever the `fe-flow` skill is installed, the runner is already on disk beside it:

- **Project-local** (the skill lives in the repo, e.g. via `npx skills` into `.claude/skills/`): `.claude/skills/fe-flow/fe-flow.sh`
- **User-level** (installed into your home agent dir): `~/.claude/skills/fe-flow/fe-flow.sh`

It computes the repo root with `git rev-parse`, so run it directly from anywhere inside the target repo — no copy step:

```bash
.claude/skills/fe-flow/fe-flow.sh ABC-123        # needs: claude, jq, gh
```
...
```bash
cp "$(find .claude ~/.claude ~/.config -path '*fe-flow/fe-flow.sh' 2>/dev/null | head -1)" scripts/fe-flow.sh
chmod +x scripts/fe-flow.sh
```

Keep `.claude/skills/fe-flow/fe-flow.sh` the source of truth; any `scripts/` copy is a wrapper you re-sync when the skill updates.
...
For each, use the fe-flow skill to take it to a pre-reviewed PR. One at a time; stop each at the PR." \
  --permission-mode acceptEdits \
...
.claude/skills/fe-flow/fe-flow.sh ABC-123 ABC-124 ABC-125
```
...
- The real safety net is **the green gate inside `fe-flow`**: no PR opens on a red build. Keep your `typecheck`/`lint`/`test`/`build` scripts honest and fast, and the autonomy stays safe.

## Graduating to CI or cloud (later, same recipe)

When local proves out, the *same* `fe-flow` skill and prompt move up the stack — only the runner and trigger change:
