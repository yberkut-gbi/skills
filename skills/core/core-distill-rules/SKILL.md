...
<!-- Generated by core-distill-rules on <date> from <N> notes. Version <n>. -->

## Before building
- Run core-grill and confirm acceptance criteria and error cases before any code.
...
## Autonomous runs
- Don't hand fe-flow an issue without acceptance criteria and error cases.
...
## How the loop closes
`core-grill` and `pm-to-prd` read this file to seed their checks, and `fe-flow` reads it at step 0. Updating it changes what the *next* feature's alignment focuses on. Point this out — the rules only matter if the skills aim at the same file.
